...
class LockerBloc extends ActionBloc<LockerEvent, LockerState, LockerAction> {
  ...
  Future<void> _onAppResumed(
    ...
    if (state.status == LockerStatus.unlocked && state.biometricState.isEnabled) {
      await _checkBiometricKeyValidity(emit);
    }
  }

  /// Checks biometric key validity and locks the app if the key has been invalidated.
  Future<void> _checkBiometricKeyValidity(Emitter<LockerState> emit) async {
    try {
      final biometricState = await _lockerRepository.determineBiometricState();
      ...
      if (biometricState.isKeyInvalidated) {
        logger.logInfo('LockerBloc: Biometric key invalidated while app was in background, locking');
        emit(state.copyWith(biometricState: biometricState));
        await _lockerRepository.lock();
      }
    } catch (error, stackTrace) {
      logger.logError(
        'LockerBloc: Failed to check biometric key validity on resume',
        error: error,
        stackTrace: stackTrace,
      );
    }
