...
class PackageInfoPlusWebPlugin extends PackageInfoPlatform {
  ...
  Uri versionJsonUrl(String baseUrl, int cacheBuster) {
    ...
    if (uri.path.length > 1 && !uri.path.endsWith('/') && (uri.isScheme('http') || uri.isScheme('https'))) {
      uri = uri.replace(path: uri.path.substring(0, uri.path.lastIndexOf('/')));
    }

    // Clean uri: remove empty path segments
    final List<String> segments = [...uri.pathSegments]..removeWhere((element) => element == '');
  ...
  Future<PackageInfoData> getAll({String? baseUrl}) async {
    ...
    final Map<String, dynamic> versionMap = await _getVersionMap(baseUrl, cacheBuster) ??
  ...
  Future<Response> _getResponse(Uri uri) async => _client == null ? await get(uri) : await _client.get(uri);
...
extension _AssetManager on AssetManager {
  ...
  String get baseUrl => getAssetUrl('').replaceAll('$assetsDir/', '');
}
