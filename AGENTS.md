...
**Backend note:** the `tm` CLI (in `C:\Users\comrade77\.local\bin`) wraps beads. The `bd` backend is NOT installed on this machine — the working backend is `br` (beads_rust). The backend is pinned in `.beads/config.yaml` via `tm.backend: br`, so plain `tm <command>` works. If that config is lost, prefix commands with `TM_BACKEND=br`.
